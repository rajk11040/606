---
title: 'RK_606_4a_Foundations for statistical inference - Sampling distributions'
author: Raj Kumar
date: 3/17/2018
output:
  html_document:
...
setwd("C:\\CUNY\\606Statistics\\Lab4a")
...
##### The population is skewed to right and is unimodal. Greater number of  house areas are in the lower end of the distribution.
...
##### It is not comparable with distribution of population. This sample is bimodal, and it is also skewed to the right. 
...
##### It is different from the mean. Samp1 has lower mean than samp2. The sample with the greater size of 1,000 will be more accurate. This is because of the Law of Averages. The bigger sample is more representative of the true composition of the population.
...
```
##### The length is 5000 which mean there are 5000 samples done for 50 areas. The distribution is going to more normal and representative if 50,000 samples are done instead of 5,000. 
...
##### The sample below has 100 elements and these represent the sample mean for the 100 iterations done to get a sample of the area.
```{r}
...
##### As the Sample becomes larger, more items come in center so that becomes larger and spread reduces.
...
##### The Sample mean is good way to estimate the mean price of the population 
```{r}
sample_means50 <- rep(NA, 5000)
...
##### Distribution 3 has the smaller spread than distribution 2 due to larger sample size. It is more desirable, in making estimates of a true value, to have a smaller spread than a bigger spread. The smaller the spread the closer samples are to the center (true value, population mean). 
