---
title: 'RK_606_4b_Foundations for statistical inference - Confidence intervals'
author: Raj Kumar
date: 3/17/2018
output:
  html_document:
...
setwd("C:\\CUNY\\606Statistics\\Lab4b")
...
##### The distribution appears normal, with an average of 1500 and the most of the data being between 1000 and 2000. I interpret typical to be a mean or a range of data that most of the data falls between.


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

##### I would expect another student's distribution to look very different to mine, because we are taking very small, randomized, and highly variable samples.
...
##### We must have independent and randomly selected values. THe total value count is greater than 30.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

##### We are 95% confident that the average population of Ames, Iowa is between 1393.44 and 1761.06 people.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```
##### The red lines are the confidence intervals that do not contain the true population mean. It appears to be roughly 95% confidence.
...
```{r}
lower_90 <- samp_mean - c90 * samp_sd / sqrt(n) 
upper_90 <- samp_mean + c90 * samp_sd / sqrt(n)
c(lower_90[1], upper_90[1])

```

```{r}
plot_ci(lower_90, upper_90, mean(population))
